...
import subprocess
import json
from utils import find_openssl_bin
#
...
def cms_verify(formatted, signing_cert_file_name, ca_file_name, logger):
    """Verifies the signature of the contents IAW CMS syntax."""
    logger.debug("verifies formatted: %s" % formatted)
    try:
        process = subprocess.Popen([OPENSSL_BIN, "cms", "-verify",
                                                "-certfile",
        ...
        logger.error("Keystone cms err: %s" % error)
    logger.debug('retcode:%s error:%s' % (retcode,err))
    if retcode:
        logger.error('Error in cms_verify: %s' % err)
        raise subprocess.CalledProcessError(retcode,"openssl", output=err)
    logger.debug("Keystone cms output: %s" % output)
    return output
...
def verify_token(token, signing_cert_file_name, ca_file_name, logger):
...
def cms_sign_text(text, signing_cert_file_name, signing_key_file_name, logger):
    """Uses OpenSSL to sign a document
    Produces a Base64 encoding of a DER formatted CMS Document
    http://en.wikipedia.org/wiki/Cryptographic_Message_Syntax
    """
    logger.debug("Keystone in cms_sign_text")
  #  cmd = "openssl cms -sign -signed %s -inkey %s -outform PEM -nosmimecap -nodetach -nocerts -noattr" % (signing_cert_file_name, signing_key_file_name)
  #  logger.debug("CDM: %s" % cmd)
  #  os.sys(cmd)
    try:
        process = subprocess.Popen([OPENSSL_BIN, "cms", "-sign",
                                               ...
                                               stdout=subprocess.PIPE,
                                               stderr=subprocess.PIPE)
        output, err = process.communicate(json.dumps(text))
        retcode = process.poll()
    except Exception as error:
       # print("Keystone cms err: %s" % error)
        logger.debug("Keystone cms err: %s" % error)
    logger.debug("Keystone finished openssl with res: %s" % retcode)
    if retcode or "Error" in err:
        if retcode == 3:
            logger.error("Signing error: Unable to load certificate - "
                      "ensure you've configured PKI with "
                      "'keystone-manage pki_setup'")
        else:
            logger.error('Signing error: %s' % err)
        raise subprocess.CalledProcessError(retcode, "openssl")
    return output


def cms_sign_token(text, signing_cert_file_name, signing_key_file_name, logger):
    #logger.debug("Keystone in cms_sign_token")
    output = cms_sign_text(text, signing_cert_file_name, signing_key_file_name,logger)
